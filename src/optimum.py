...
def fetch_data(fpath="../../gaiadr3/combined_trainingset2_with_xp_norm.ONE.parq"):
    ...
    df=pd.read_parquet(fpath)#.head(10000)
    return df
...
label = pred_vec[ii]
rawdata = fetch_data()
data=prepare_data(rawdata,label,random_state=42,frac=0.5)
print(len(data))
...
def train_sh23_model(data,params,label="target",mlflow_experiment_id=1):
    ...
    with mlflow.start_run(experiment_id=mlflow_experiment_id,run_name=f"{label}"):
       
        model = xgb.train(params, dtrain, num_boost_round=params['n_estimators'],verbose_eval=True,
...
params={ 'colsample_bylevel': 1.0, 
        'colsample_bynode': 0.99, 'colsample_bytree': 0.33, 
        'gamma': 0.17, 'learning_rate': 0.32, 
        'max_depth': 4, 
        'min_child_weight': 1, 
        'n_estimators': 432, 'reg_alpha': 0.03, 
        'reg_lambda': 0.27, 'subsample': 1.0, 'tree_method': 'gpu_hist'}
...
# Load data
#data = xgb.DMatrix('path/to/data')

# Split the data into training and validation sets
#X_train, X_val, y_train, y_val = train_test_split(
#    data.data, data.get_label(), test_size=0.2, random_state=42)
...
space = {
    ...
    'max_depth': hp.choice('max_depth', range(5, 12)),
    'learning_rate': hp.quniform('learning_rate', 0.05, 0.9, 0.05),
...
#    'subsample_freq': hp.choice('subsample_freq', range(1, 10)),
    ...
    'tree_method': 'gpu_hist',#hp.choice('tree_method', ['auto','gpu_hist' ]),#'exact', 'approx', 'gpu_hist']),
